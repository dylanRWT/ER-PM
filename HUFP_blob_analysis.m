function HUFP_blob_analysis(blob_count,blob_feats,sm_fm,folder_name,base_file_name,px_sz)
    ...
    if blob_count > 0       % if there are any blobs to do this procedure on 
        ...
        if length(decent_blob_fits) ~= 1 && decent_blob_fits(1) ~= 0 % If there are still blob features to evaulate 
            ...
            fig2_name = strcat(folder_name,'HUFP outputs/isolated images/',base_file_name,"_well_fitted_blobs.pdf") ; 
        ...
        if decent_found == 1 && height(decent_blobs) >= 5
            ...
            %
            set(gcf, "units", 'inches', 'position', [2 2 20 4])
            fig2_name = strcat(folder_name,'HUFP outputs/isolated images/',base_file_name,"_GMM_blobs.pdf") ; 
            saveas(gcf, fig2_name,'pdf')
        else
            g_num = 0;
        end
        %
        try
            writematrix(decent_blobs(:,1), strcat(folder_name,'HUFP outputs/isolated data/',base_file_name,'_origin.csv')) ; 
        catch
            writecell(decent_blobs(:,1), strcat(folder_name,'HUFP outputs/isolated data/',base_file_name,'_origin.csv')) ; 
        end
        %
        eq = zeros((1+g_num),6);
        ...
        writetable(eq, strcat(folder_name,'HUFP outputs/isolated data/',base_file_name,'_stat.csv')) ; 
    else
