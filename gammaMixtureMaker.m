...
clc
%
diroutput = dir('.');                   % look at all of the folders in the current directory
...
mastertable = table('Size',[height(dirf)*3,22],'VariableTypes', ...
...
ub = [300,30,400,1e6,800,15,350,1e6];
...
for folder = 1:height(dirf)
    ...
    for w = 1:width(files)
        close all
        data = readmatrix(strcat(dataset,'/',files{1,w}));
        images = width(data);
        ...
        plot(x,yout,'k','LineWidth',2)
        subplot(1,4,4)
        histogram(pooleddata,'BinEdges',0:25:2000), hold on
        plot(x,gaMM1(solcoeff(1:4)),'r-')
        xline(AI,'r-','LineWidth',2)
        xline(medianA,'k--','LineWidth',2)
        xline(meanA,'k:','LineWidth',2)
        plot(x,gaMM1(solcoeff(5:8)),'g-')
        xline(BI,'g-','LineWidth',2)
        xline(medianB,'k--','LineWidth',2)
        ...
        histogram(pooleddata,'BinEdges',0:25:2000), hold on
        plot(x,gaMM1(solcoeff(1:4)),'r-')
        xline(XI,'k-','LineWidth',2)
        xline(medianX,'k--','LineWidth',2)
...
writetable(mastertable,"masterNNDtableGamma.csv");
%
sum(mastertable{:,5})