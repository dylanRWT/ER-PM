%% HUFP_peak_detection
% A function used in HUFP_runone to classify NNMF outputs into ladder or
% blob features, also generates a smoothed version of the data sets as well as indicies where peaks were found.                 
function [sm_fm,pk_fm,ladder_feats,blob_feats,neither_feats,blob_count] = HUFP_peak_detection(x0,raw,feat_num,feat_per_cell)
   ...
   %
    ...
    if (jam  == 'Y') || (jam == 'y')
        ...
        % super ladder plotter
        prompt = "enter 'y' to plot superladder features    " ; 
        jam2 = input(prompt,'s') ; 
        if (jam2  == 'Y') || (jam2 == 'y')
            for i = 1:width(super_ladder_feats)
                    feat = super_ladder_feats(1,i);
                    figure('Name','SuperLadder feature')
                    plot(x0, raw(:,feat), 'r'), hold on 
                    plot(x0, sm_fm(:,feat), 'b')
                    ylim([0 (1.1*max(raw(:,feat)))])
                    temp_peaks = pk_fm(:,feat)>0 ;
