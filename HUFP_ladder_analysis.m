...
function [best_period_px,decent_displacements,fixed_rungs,rung_sum,initialValueTable,best_model,displacements] = HUFP_ladder_analysis(x0,pk_fm,ladder_feats,lwcp,upcp, folder_name, base_file_name, px_sz)  
    ...
    writetable(initialValueTable, strcat(folder_name,'ladder data/model coefficients/',base_file_name,'_coeff.csv'),'WriteRowNames' , true) 
    ...
    writetable(best_model, strcat(folder_name,'ladder data/model fit/',base_file_name,'_',num2str(BM),'_fit.csv'),'WriteRowNames',true) ;
    ...
    writetable(residual_info,strcat(folder_name,'ladder data/model fit/',base_file_name,'_',num2str(BM),'_resinfo.csv'),'WriteRowNames',true)
    ...
    fig1_name = strcat(folder_name,'ladder data/model fit/',base_file_name,".pdf");
