%% gammaMM_runone
%
% used to read a single data set fit it with a user specified number of
% gamma distributions and output data on the fit. 
%
...
clear 
clc
% select a the files to analylize 
[file,path] = uigetfile("../*.csv",'multiselect','on');
filecount = 1; % this is a counter for the table index
runcols = input("Do you want to run individual columns as a seperate fit? 1 or 0    :");
...
mastertable = table('Size',[1,22],'VariableTypes', ...
...
%% use a for loop to scan each file
    for w = 1:width(file)
        dataset = extractBefore(file{w},'.csv');
        data = readmatrix(strcat(path,file{w}));
        images = width(data);
        ...
        yorig = h1.Values;
        ch_pair = extractBefore(file{w},'.csv');
        if length(ch_pair) == 2 || length(ch_pair) == 3
        ...
        figure('Name',strcat(dataset,"_",ch_pair))
        subplot(1,3,1)
        yerr = yorig-yout;
        histogram(yerr)
        [h,p] = kstest(yerr);
        ...
        saveas(gcf,strcat(path,dataset,'.pdf'))
        %
        else
        beta = [10,20];
        alpha = [15,20];
    ...
    %%
userinput = input("What should the master table be saved as? ",'s');
writetable(mastertable,strcat(path,userinput))